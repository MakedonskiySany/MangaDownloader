...
namespace WindowsFormsApplication2
{
    public partial class Form1 : Form
    {
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
           ...
           //     i++;
            ...
            else
            {
                ...
                if (site_link.IndexOf("henchan.me") != -1)
                {
                    if (convert_cbr.Checked == true) dirInfo.CreateSubdirectory(@"cbr");//создана папка для глав cbr
                    folder_cbr_name = path + @"\cbr";
        ...
        private void Search_parts_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (site_link.IndexOf("mangafox.me") != -1)
                    mangafox_searchparts(site_link);
                if (site_link.IndexOf("mangatown.com") != -1)
                    mangatown_searchparts(site_link);
        ...
        public void mangatown_searchparts(string site_link)
        {
            {
                ...
                while ((start = HTML_first_page.IndexOf("<li>\r\n                    <a", start + 1)) != -1)
                {
        ...
        public void mangafox_searchparts(string site_link)
        {
            {
                int start = 0;//стартовая позиця для indexof шаг1
                int buf_start = 0;//буферный счетчик. начало
                ...
                //поиск подстроки с названием и ссылкой
                while ((start = HTML_first_page.IndexOf("></span></a>", start + 1)) != -1)
                {
                    length = HTML_first_page.IndexOf("</div", start) - start;
                    text = HTML_first_page.Substring(start, length);//блок с сылкой и названием
                    buf_start = text.IndexOf("<a href=");
                   ...
                   //поиск ссылок и названий глав
                    ...
                    {   
                        ...
                        while ((start = HTML_first_page.IndexOf("<div class=\"related\">", start + 1)) != -1)
                        {
                            ...
                            link = "http://henchan.me/online/" + text;//ссылка на онлайн читалку для одной главы
                            Console.WriteLine(link);
    ...
    public class Func_saver//сюда будут писаться всякие функции
    {
        ...
        public string get_HTML(string url)
        {
            ...
            //string encoding = resp.Headers["Content-Type"].ToString().Split(new string[] { "charset=" }, StringSplitOptions.RemoveEmptyEntries)[1];
